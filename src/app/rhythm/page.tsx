      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Bookplate className="p-4">
          {loading ? <Skeleton className="h-10 w-24" /> : (
            <MiniEntry icon="calendar" value={stats!.booksThisYear} label="books this year" />
          )}
        </Bookplate>
        <Bookplate className="p-4">
          {loading ? <Skeleton className="h-10 w-24" /> : (
            <MiniEntry icon="flame" value={stats!.dayStreak} label="day streak" />
          )}
        </Bookplate>
        <Bookplate className="p-4">
          {loading ? <Skeleton className="h-10 w-24" /> : (
        ...
        <Bookplate className="p-4">
          {loading ? <Skeleton className="h-10 w-24" /> : (
            <MiniEntry icon="book" value={stats!.totalBooks} label="total books" />
          )}
        </Bookplate>
      </div>
